...
def parse_file(file_name: str) -> List[Tuple[str, str]]:
    ...
    for line_number, typo, correction in parse_file_lines(file_name):
        if typo in typos:
            cli.log.warning(
                '{fg_red}Error:%d:{fg_reset} Ignoring duplicate typo: "{fg_cyan}%s{fg_reset}"',
                line_number,
                typo,
            )
            continue
        ...
        if not (all([c in TYPO_CHARS for c in typo])):
            cli.log.error(
                '{fg_red}Error:%d:{fg_reset} Typo "{fg_cyan}%s{fg_reset}" has characters other than a-z, \' and :.',
                line_number,
                typo,
            )
            maybe_exit(1)
        for other_typo in typos:
            if typo in other_typo or other_typo in typo:
                cli.log.error(
                    '{fg_red}Error:%d:{fg_reset} Typos may not be substrings of one another, otherwise the longer typo would never trigger: "{fg_cyan}%s{fg_reset}" vs. "{fg_cyan}%s{fg_reset}".',
                    line_number,
                    typo,
                    other_typo,
                )
        ...
        if len(typo) < 5:
            cli.log.warning(
                '{fg_yellow}Warning:%d:{fg_reset} It is suggested that typos are at least 5 characters long to avoid false triggers: "{fg_cyan}%s{fg_reset}"',
                line_number,
                typo,
            )
        if len(typo) > 127:
            cli.log.error(
                '{fg_red}Error:%d:{fg_reset} Typo exceeds 127 chars: "{fg_cyan}%s{fg_reset}"',
                line_number,
                typo,
            )
            maybe_exit(1)
...
def check_typo_against_dictionary(typo: str, line_number: int, correct_words) -> None:
    ...
    if typo.startswith(":") and typo.endswith(":"):
        if typo[1:-1] in correct_words:
            cli.log.warning(
                '{fg_yellow}Warning:%d:{fg_reset} Typo "{fg_cyan}%s{fg_reset}" is a correctly spelled dictionary word.',
                line_number,
                typo,
            )
        ...
        for word in correct_words:
            if word.startswith(typo[1:]):
                cli.log.warning(
                    '{fg_yellow}Warning:%d: {fg_reset}Typo "{fg_cyan}%s{fg_reset}" would falsely trigger on correctly spelled word "{fg_cyan}%s{fg_reset}".',
                    line_number,
                    typo,
                    word,
                )
...
def encode_link(link: Dict[str, Any]) -> List[int]:
    ...
    if not (0 <= byte_offset <= 0xFFFF):
        cli.log.error(
            "{fg_red}Error:{fg_reset} The autocorrection table is too large, a node link exceeds 64KB limit. Try reducing the autocorrection dict to fewer entries."
        )
        maybe_exit(1)
...
@cli.argument(
    "-kb",
    "--keyboard",
    type=keyboard_folder,
    completer=keyboard_completer,
    help="The keyboard to build a firmware for. Ignored when a configurator export is supplied.",
)
@cli.argument(
    "-km",
    "--keymap",
    completer=keymap_completer,
    help="The keymap to build a firmware for. Ignored when a configurator export is supplied.",
)
...
@cli.argument(
    ...
    arg_only=True,
    action="store_true",
    help="Quiet mode, only output error messages",
)
...
def generate_autocorrect_data(cli):
    ...
    current_keyboard = (
        cli.args.keyboard
        or cli.config.user.keyboard
        or cli.config.generate_autocorrect_data.keyboard
    )
    current_keymap = (
        cli.args.keymap
        or cli.config.user.keymap
        or cli.config.generate_autocorrect_data.keymap
    )

    if current_keyboard and current_keymap:
        cli.args.output = (
            locate_keymap(current_keyboard, current_keymap).parent
            / "autocorrect_data.h"
        )
